...
function getPresetExt(colorName, palette) {
    if (colorName === 'noir') {
        return {
            semantic: {
                primary: {
...
export function applyThemeConfig(config) {
    ...
    const colorName = config.primary_color || 'blue';
    const surfaceName = config.surface || 'slate';

    const presetObj = presets[presetName] || presets.Aura;
    const palette = primaryColors[colorName] || primaryColors.blue;
    const surface = surfacePalettes[surfaceName] || surfacePalettes.slate;

    $t()
        .preset(presetObj)
        .preset(getPresetExt(colorName, palette))
